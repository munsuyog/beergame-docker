...
const SettingsTeam = () => {
  ...
  const { gameSetup, loading, error } = useSelector(
    (state) => state.game
  );
  ...
  useEffect(() => {
    const fetchGameSetup = async () => {
      startLoading();
      try {
        await dispatch(getGameSetup(teamId)).unwrap();
      } catch (error) {
        console.error("Failed to fetch game setup:", error);
      } finally {
        stopLoading();
      }
    };
  ...
  useEffect(() => {
    if (gameSetup) {
      setLocalGameSetup(gameSetup);
      if (gameSetup.demands && gameSetup.demands.length > 0) {
        const demandData = gameSetup.demands[0].demand;
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      await dispatch(
        saveGameSetup({ gameId: teamId , gameData: formattedSetup })
      ).unwrap();
      await dispatch(getGameSetup(teamId)).unwrap();
    } catch (error) {
